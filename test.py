            ...
            if (sum<value) & ((sum*10/value)>8):
                for record_check in tx.run("MATCH (p1:Account)-[tx1:TX {tx_hash: $tx_hash}]->(p2:Account)-[tx2:TX]->(p3:Account) where tx2.value > 10 RETURN p1.addr, p2.addr, p3.addr, tx1.tx_hash, tx2.tx_hash, tx1.value, tx2.value, tx1.timestamp, tx2.timestamp", tx_hash=hash):
                    ...
                    if i==0:
                        ...
                        tx.run("MATCH (a:Account {addr: $addr1, check: True})"
                               "MATCH (b:Account {addr: $addr2, check: True})"
                               "MATCH (a)-[tx:TX {tx_hash: $tx_hash, value: $value, from_addr: $from_addr, to_addr: $to_addr, timestamp: $timestamp}]->(b)"
                               "SET tx.weight=$weight", addr1=record_check[0], addr2=record_check[1], tx_hash=record_check[3], value = record_check[5], from_addr = record_check[0], to_addr = record_check[1], timestamp = record_check[7], weight=record_tx["tx.weight"]+1)
                    ...
                    if i==0:
                        ...
                        tx.run("MATCH (a:Account {addr: $addr1, check: True})"
                               "MATCH (b:Account {addr: $addr2, check: True})"
                               "MATCH (a)-[tx:TX {tx_hash: $tx_hash, value: $value, from_addr: $from_addr, to_addr: $to_addr, timestamp: $timestamp}]->(b)"
                               "SET tx.weight=$weight", addr1=record_check[1], addr2=record_check[2],  tx_hash=record_check[4], value = record_check[6], from_addr = record_check[1], to_addr = record_check[2], timestamp = record_check[8], weight=record_tx["tx.weight"]+float(1/i))           
